...
class Stereocilia(Scene):
    def construct(self):
        ...
        outer_ear = VGroup(
            ear[1],
            ear[18:21] , ear[22:25] , ear[29:41] , ear[45:48] , ear[54:]
        )
        ...
        self.play(
            Write(eustachian_tube),
            Create(eustachian_arrow)
        )
        self.wait()
        self.play(
            Unwrite(eustachian_tube, reverse=False),
            Uncreate(eustachian_arrow)
        )
        self.play(
...
class Test(Scene):
    def construct(self):
        ear = SVGMobject("human_ear")
        ear.set(width= config.frame_width)
        label = index_labels(ear)
        self.add(ear, label)
