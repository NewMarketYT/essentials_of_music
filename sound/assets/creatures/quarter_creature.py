...
class QuarterCreature(SVGMobject):
    ...
    def align_data(self, mobject):
        ...
        if isinstance(mobject, QuarterCreature):
            self.mode = mobject.get_mode()

    def name_parts(self):
        self.boundingbox = self.submobjects[BOUNDING_BOX]
        self.mouth = self.submobjects[MOUTH_INDEX]
        self.body = VGroup(
            *[self.submobjects[TAIL_INDEX], self.submobjects[BODY_INDEX]]
        )
        self.pupils = VGroup(
...
def get_all_creature_modes():
    result = []
    prefix = "%s_" % QuarterCreature.CONFIG["file_name_prefix"]
    suffix = ".svg"
